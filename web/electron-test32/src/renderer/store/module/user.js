import { loginByUsername, logout, getUserInfo, getShopInfo, getUserData,
  getPowerClass, ChangePwd, GetVeriCode, ChangePhone, FindPwd, GetTipInfo, GetTechList, GetpoweList} from '@/api/login'
import { getToken, setToken, removeToken, setCookies } from '@/utils/auth'
...
const user = {
  ...
  actions: {
      ...
      return new Promise((resolve, reject) => {
        loginByUsername(username, userInfo.password, userInfo.shopserialnum).then(response => {
          ...
          if (data.RetCode === 1) {
            ...
            setCookies('code', JSON.parse(objData).shopid)
            setCookies('account', JSON.parse(objData).adminid)
            resolve()
          }
        }).catch(error => {
          reject(error)
    ...
    GetUserInfo ({ commit, state }) {
      return new Promise((resolve, reject) => {
        commit('SET_ROLES', 'wucheng')
        // } else {
        //   reject(new Error('getInfo: roles must be a non-null array !'))
        // }
        commit('SET_NAME', 'wucheng')
        commit('SET_AVATAR', '13265591725')
    ...
    queryTechStatusInfo ({commit, state}, requestParams) {
      return new Promise((resolve, reject) => {
        GetTechList(requestParams).then((response) => {
    ...
    queryTechPoweInfo ({commit, state}, requestParams) {
      return new Promise((resolve, reject) => {
        GetpoweList(requestParams).then((response) => {
          const ResObj = response.Data
          resolve(ResObj)
        }).catch((error) => {
          reject(error)
