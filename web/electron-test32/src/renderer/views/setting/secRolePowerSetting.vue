  ...
  <div>
    ...
    <div>
      <div style="margin-left: 10px" v-for="item in totalData.data" :label="item.lebel" :key="item.id">
        ...
        <div style="margin-left: 100px" v-for="item in item.data" :label="item.lebel" :key="item.id">
    ...
    <div style="margin-left: 100px;vertical-align:middle;">
      <div v-for="item in shops" :label="item.Title" :key="item.ID">
        <p><img style="margin-right: 10px;cursor: pointer;" @click="clickDel(item.ShopCode)" src="http://r.handnear.com/chain/img/hongcha.png">{{item.Title}}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    getPowers () {
      this.$store.dispatch('queryRolePowers', {ID: this.id}).then((response) => {
        this.setDate(response.ThisClass)
        this.shops = []
        response.ShopInfoListForRole.forEach(element => {
          var obj = {}
    ...
    clickConfirm () {
      ...
      this.$store.dispatch('setRolePowers', {RoleID: this.id, chkPowerID: arr, ShopID: shopIDs}).then((response) => {
        this.$message({
          type: 'success',
          message: '修改成功'
        })
