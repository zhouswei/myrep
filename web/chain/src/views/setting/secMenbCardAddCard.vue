    ...
    <div>
      ...
      <p class="infoPerLine1">
        ...
        <span class="infoShowName margin">适用门店：</span>
          <el-transfer :titles="['全选', '全选']" class="transfer_wrap" @change="useShopChange" v-model="checkUseShop" :data="shopsUseData"></el-transfer>
      </p>
      <p class="infoPerLine1">
        <span class="infoShowName">可用卡门店：</span>
        <span class="infoShowName margin">适用门店：</span>
          <el-transfer :titles="['全选', '全选']" class="transfer_wrap" @change="sendShopChange" v-model="checkSendShop" :data="shopsSendData"></el-transfer>
      </p>
  ...
  methods: {
    ...
    dataValidation () {
      ...
      if (this.para.SaleCommType !== 0) {
        if (this.para.SaleCommType === 1 || this.para.SaleCommType === 2) {
          if (this.para.SaleComm > 100 || !(/^\d+(\.\d{1,2})?$/.test(this.para.SaleComm))) {
            this.$message.error('提成数请输入两位小数以内的正数')
            return false
          }
        }
      }
...
<style scoped>
  @media (max-width:500px) {
    .transfer_wrap >>> .el-transfer-panel {
      width: 152px;
    }
    .transfer_wrap >>> .el-transfer__buttons {
     padding: 0 5px;
    }
  }
</style>
