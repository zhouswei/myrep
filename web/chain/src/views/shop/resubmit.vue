...
export default {
  ...
  methods: {
    update () {
      this.loading = true
      this.$store.dispatch('queryEditSnapInfo', {id: this.ID}).then((response) => {
        this.para = response.info
        this.GroupID = response.info.GroupID
        this.shopTitle = response.info.ShopCode
        this.shopTitle = response.info.Title
        this.shopContact = response.info.Manager
        this.ContactPhoneNo = response.info.Phone
        this.Email = response.info.Email
        this.space = response.info.Space
        this.pro7 = getShopType(response.info.ChainType)
        this.AccountMax = response.info.AccountMax
        this.SMSMax = response.info.SMSMax
        this.useableDate = parseTime(formatTimestamp(response.info.ServiceEndDate), '{y}-{m}-{d}')
        ...
        if (response.info.ShopHours.indexOf('~') !== -1) {
          ...
          if (strArr[0] !== '') {
            this.queryStartDate = new Date(2016, 9, 10, strArr[0].split(':')[0], strArr[0].split(':')[1])
          } else {
            this.queryStartDate = ''
          }
          if (strArr[1] !== '') {
            this.queryEndDate = new Date(2016, 9, 10, strArr[1].split(':')[0], strArr[1].split(':')[1])
          } else {
            this.queryEndDate = ''
          }
        } else {
    ...
    clickSave () {
      if (this.dataValidation()) {
        ...
        if (this.queryStartDate !== null) {
          startStr = parseTime(this.queryStartDate, '{h}:{i}:{s}')
        }
        var endStr = ''
        if (this.queryEndDate !== null) {
          endStr = parseTime(this.queryEndDate, '{h}:{i}:{s}')
        }
        this.para.ShopHours = startStr + '~' + endStr
        this.$store.dispatch('queryEditSnapSpecMod', {
          ID: this.ID,
          GroupID: this.GroupID,
          Title: this.shopTitle,
          Pic: this.pic,
          Space: parseInt(this.space) === 0 ? '' : parseInt(this.space),
