      ...
      <el-tab-pane label="营收统计" name="first">
        <div v-if="isPermission(22)">
          ...
          <el-table show-summary border :data="totallist" :header-cell-style="headercellyangshi">
            ...
            <el-table-column
              label="门店应收统计(元)"
              label-class-name="门店应收总计 =营业应收金额 + 会员应收金额"
            ...
            <el-table-column
              label="门店实收统计(元)"
              label-class-name="门店实收总计 =营业实收金额 + 会员实收金额"
              :formatter="formatfloat2StringKeep2"
            ...
            <el-table-column
              label="会员应收金额(元)"
              :formatter="formatfloat2StringKeep2"
              label-class-name="会员应收金额(元)=会员开卡应收+会员充值应收-撤销开卡应收-撤销充值应收-退卡金额"
            ...
            <el-table-column
              ...
              :formatter="formatfloat2StringKeep2"
              label="会员实收金额(元)"
              label-class-name="会员实收金额(元)=会员开卡实收+会员充值实收-撤销开卡实收-撤销充值实收-退卡金额"
    ...
    queryDataHandle (shopid, beginDate, endDate) {
      ...
      this.$store.dispatch('queryBusinessStatisticData', { ShopID: shopid, BeginDate: beginDate, EndDate: endDate }).then((response) => {
        ...
        response.listInfo.forEach(element => {
          ...
          tmpObj.data = []
          tmpObj.data.push({title: '营业实收金额', value: element.RealPrice})
