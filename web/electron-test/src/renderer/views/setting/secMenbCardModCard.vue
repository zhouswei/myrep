...
export default {
  ...
  methods: {
    ...
    dataValidation () {
      ...
      if (!this.isPositiveInteger(this.para.Discount) || this.para.Discount <= 0 || this.para.Discount > 100) {
      ...
      if (isNaN(this.para.ActiveMonth) || this.para.ActiveMonth <= 0 || !this.isPositiveInteger(this.para.ActiveMonth)) {
        this.$message.error('会员卡有效期只能输入正整数')
        return false
      }
      if (this.para.IsScore && this.UCardPayScore === '' && this.UCardFillScore === '') {
        this.$message.error('充值和消费金额不能同时为空')
        return false
      }
      if (this.para.IsScore && this.para.UCardPayScore === 0) {
        this.$message.error('积分规则数据不能为0')
        return false
      }
      if (this.para.IsScore && this.para.UCardFillScore === 0) {
        this.$message.error('积分规则数据不能为0')
        return false
      }
