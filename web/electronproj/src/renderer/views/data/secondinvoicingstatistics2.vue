...
export default {
  ...
  methods: {
    ...
    clickExport () {
      this.$store.dispatch('queryStoreTableDetailData', this.data3).then((response) => {
        this.tableData = []
        response.listInfo.forEach(element => {
          var ItemObj = {}
          ItemObj.pro1 = element.CardNo
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['会员卡号', '会员姓名', '消费门店', '扣款总金额(元)', '实扣总金额(元)', '营业日期', '消费时间', '操作员']
          const filterVal = ['pro1', 'pro2', 'pro3', 'pro4', 'pro5', 'pro6', 'pro7', 'pro8']
